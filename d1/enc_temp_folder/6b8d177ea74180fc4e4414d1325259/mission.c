...
int select_mission(int anarchy_mode, char* message, int (*when_selected)(void))
{
	...
	for (int i = 0; i < mission_count; i++) {
		...
		Ranking.missionRanks[i] = rankPoints / (Current_mission->last_level - Current_mission->last_secret_level); // If all levels in the current mission are completed, an aggregate rank set here will show next to it.
